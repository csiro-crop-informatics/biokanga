// RNAseqSitePotential.cpp : Defines the entry point for the console application.
// Used to generate RNAseq transcriptome wide read start site potentials
#include "stdafx.h"
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
typedef struct TAG_sSiteOcts {
	int Insts;							// number of intstances of this octamer
	int Cnts;							// total number of reads 
} tsSiteOcts;

tsSiteOcts *m_pSiteOcts;				// to hold read start site octamer instance + read cnts
...
struct arg_file *infile = arg_file1("i","in","<file>",			"input from this alignreads generated BED or CSV file");
...
if (help->count > 0)
        {
		printf("\n%s RNAseq Transcriptome Read Start Site Potentials, Version %s\nOptions ---\n", gszProcName,cpszProgVer);
...
// show user current resource limits
	...
	{
	printf("\n%s Predict RNAseq read start site potential from aligned Reads, Version %s\n",gszProcName,cpszProgVer);
...
if((m_pSiteOcts = new tsSiteOcts [0x10000])==NULL)
	{
...
// iterate reads (will have been sorted by chrom + start loci)
memset(m_pSiteOcts,0,sizeof(tsSiteOcts) * 0x010000);
pRead = NULL;
szCurChrom[0] = '\0';
CurReadStart = -1;
while((pRead = IterSortedReads(pRead,true))!=NULL)
	{
	if(szCurChrom[0] == '\0' || stricmp(szCurChrom,pRead->szChromName))
		{
		...
		strcpy(szCurChrom,pRead->szChromName);
		CurReadStart = -1;
		}
	...
	if((Rslt=m_pBioseq->GetData(ChromID,eSeqBaseType,Ofs,Seq,8)) != 8)
		{
...
//		CSeqTrans::ReverseComplement((unsigned int)8,pRead->Seq);
	...
	if(OctIdx >= 0)
		...
		if(Ofs != CurReadStart)
			{
			m_pSiteOcts[OctIdx].Insts += 1;
			CurReadStart = Ofs;
			}
		m_pSiteOcts[OctIdx].Cnts += 1;
		}
	}
...
for(OctIdx = 0; OctIdx <= 0x0ffff; OctIdx++)
	{
	if(m_pSiteOcts[OctIdx].Insts == 0)
		Ratio = 0.0;
	else
		Ratio = (double)m_pSiteOcts[OctIdx].Cnts/m_pSiteOcts[OctIdx].Insts;
	BuffIdx += sprintf(&szBuff[BuffIdx],"\"%s\",%d,%d,%8.8f\n",StepIdx2Seq(8,OctIdx),m_pSiteOcts[OctIdx].Insts,m_pSiteOcts[OctIdx].Cnts,Ratio);
