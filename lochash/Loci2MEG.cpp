// Loci2MEG.cpp : Defines the entry point for the console application.
...
const int cMAFminSpecies = 2;				// minimum number of species that can be handled
...
const int cDfltCoreLen= 20;				// if core lengths not specified then default to cDfltMinCoreLen
const int cMaxCoreLen = 5000000;		// maximum core lengths can be specified upto this length
...
const int cMaxTotMEGseqLen = 0x0fffffff; // allow total of all MEG concatenated sequences to be at most this many bases

// processing mode
typedef enum eProcMode {
	eProcModeMEG3,						// output concatenated species sequences as MEG3
	eProcModeFasta,						// output concatenated species sequences as fasta with InDel/Unaligned markers removed
	eProcModeMultiFastaAlignSeqs,		// output individual species sequences as multifasta with InDels and Unaligned bases representated as InDels
	eProcModeMultiFastaSeqs,			// output individual species sequences as multifasta with InDel/Unaligned markers removed
} etProcMode;
...
typedef struct TAG_sProcParams 
	...
	char szMEGFile[_MAX_PATH];		// output MEG file to create/write
	int hOutFile;					// opened file handle for output MEG file
	int MEGLen;						// current length of each MEG sequence
	int MaxMEGseqLen;				// MEG sequences can contain at most this many bases before being truncated
	int MEGBlockStart;				// length when block was started - used for checkpointing in case rollback required
	unsigned char *pMEGSeqs[cMaxAlignedSpecies];  // ptrs to each MEG sequence
	bool bMEGLenTrunc;				// true if concatenated MEG sequences are being truncated because overlength 
	...
	int hRsltsFile;					// write results into this MEG file
	...
	char szRegionFile[_MAX_PATH];	// region charaterisation biobed file
	...
	int FiltUnderLenCnt;			// number of loci filtered because of underlength
	int FiltOverLenCnt;				// number of loci filtered because of overlength
				...
				etLociFileType LociFileType,		// expected loci file type
		 		...
		 		char *pszFilterRefIDFile, // exclude any RefIDs in this filter file
 				 char *pszMAFFile,				// multiple alignment file
					char *pszOutputFile,	// where to write out MEG file
...
int AddLoci(char *pszChrom,char Strand,int StartLoci,int Len,tsProcParams *pParams);
...
int EndAlignBlock(char *pszRefChrom,int StartLoci,tsProcParams *pProcParams);
...
int WriteMEGFile(tsProcParams *pProcParams);
...
struct arg_int  *ProcMode = arg_int0("x","procmode","<int>",		"processing output to file as 0: MEG3, 1: Fasta sequences, 2: Multifasta sequences, 3: Multifasta alignments (default = MEG3)");
...
struct arg_int  *MaxCoreLen = arg_int0("M","maxcorelen","<int>",	"only process elements <= this length (default = 5000000)");
...
void *argtable[] = {help,version,FileLogLevel,ScreenLogLevel,LogFile,
					...
					ProcMode,LociFileType,Region,RegionFile,ExcludeFile,IncludeFile,IncludeChroms,ExcludeChroms,
					end};
...
if (!argerrors)
	...
	iProcMode = ProcMode->count ? ProcMode->ival[0] : eProcModeMEG3;
	if(iProcMode < eProcModeMEG3 || iProcMode > eProcModeMultiFastaSeqs)
		{
...
switch(ProcMode) {
	case eProcModeMEG3:
		return("MEG3");
	case eProcModeFasta:
		return("Fasta concatenated sequences with InDels/Unaligned removed");
	case eProcModeMultiFastaAlignSeqs:
		return("Multifasta sequences with InDels and Unaligned represented as InDels");
	case eProcModeMultiFastaSeqs:					
		return("Multifasta sequences with InDels/Unaligned removed");
	default:
		break;
	}
return("Unsupported");
...
strcpy(ProcParams.szMEGFile,pszOutputFile);
if(Region != eMEGRAny)
	strcpy(ProcParams.szRegionFile,pszRegionFile);
...
// determine max MEG sequence length for any single species which can be handled
ProcParams.MaxMEGseqLen = cMaxTotMEGseqLen/ProcParams.MaxNumSpecies;
for(Idx = 0; Idx < ProcParams.MaxNumSpecies; Idx++)
	{
	if((ProcParams.pMEGSeqs[Idx] = new unsigned char [ProcParams.MaxMEGseqLen])==NULL)
		{
	    CleanupResources(&ProcParams);
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Unable to allocate memory (%d bytes) for holding MEG sequences",ProcParams.MaxMEGseqLen);
		return(eBSFerrMem);
		}
	}

#ifdef _WIN32
	ProcParams.hOutFile = open(pszOutputFile, O_CREATETRUNC);
#else
     if((ProcParams.hOutFile = open64(pszOutputFile,O_RDWR | O_CREAT,S_IREAD | S_IWRITE))!=-1)
          ftruncate(ProcParams.hOutFile,0);
#endif

if(ProcParams.hOutFile == -1)					// check if file open succeeded
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"Unable to create/open output file '%s' - %s",pszOutputFile,strerror(errno));
	CleanupResources(&ProcParams);
...
AddLoci(char *pszChrom,char Strand,int StartLoci,int Len,tsProcParams *pParams)
{
tsCoreLoci *pNxtCore;
int ChromID;

if(Len < pParams->MinCoreLen)
	{
	pParams->FiltUnderLenCnt += 1;
	return(0);
	}
if(Len > pParams->MaxCoreLen)
	{
	pParams->FiltOverLenCnt += 1;
	return(0);
	}
...
int				// returns count of loci added (requested start/end loci may have been split into multiple loci) if region accepted, 0 if filtered out, < 0 if error
FilterRegionIn(char *pszChrom,char Strand,int StartLoci,int EndLoci,tsProcParams *pParams)
{
int Rslt;
int FeatureLen;
int CurFeatureID;
...
if(pParams->FiltRegion == eMEGRAny)
	{
	Rslt = AddLoci(pszChrom,'+',StartLoci,FeatureLen,pParams);
...
if(ChromID < 1 || (CurFeatureID = pBED->LocateFeatureIDinRangeOnChrom(ChromID,StartLoci,EndLoci,1)) < 1)
	{
	if(pParams->FiltRegion == eMEGRIntergenic)
		{
		Rslt = AddLoci(pszChrom,'+',StartLoci,FeatureLen,pParams);
		return(Rslt > 0 ? 1 : Rslt);
		}
...
switch(pParams->FiltRegion) {
	case eMEGRIntrons:		// only interested in INTRONS
		...
		for(CurIntron = 1; CurIntron <= NumIntrons; CurIntron++)
			...
			if((Rslt = AddLoci(pszChrom,'+',StartLoci+IntronStartOfs,(IntronEndOfs - IntronStartOfs) + 1,pParams)) < 0)
				return(Rslt);
			if(Rslt > 0)
				NumEls++;
			}
	...
	default:
		...
		if(pParams->FiltRegion >= eMEGRCDS)
			{
		...
		for(CurExon = 1; CurExon <= NumExons; CurExon++)
			...
			switch(pParams->FiltRegion) {
				...
				case eMEGUTR:		// only process UTRs
					...
					if(ExonStartOfs < CDSStart && ExonEndOfs > CDSEnd)
						{
						if((Rslt = AddLoci(pszChrom,'+',ExonStartOfs,CDSStart - ExonStartOfs ,pParams)) < 0)
							return(Rslt);
						if(Rslt > 0)
							NumEls++;
			...
			if(ExonStartOfs >= 0 &&  ExonEndOfs >= 0)
				...
				if((Rslt = AddLoci(pszChrom,'+',ExonStartOfs,LociLen,pParams)) < 0)
					return(Rslt);
				if(Rslt > 0)
					NumEls++;
				}
...
int NumSubEls;		// original elements may be split into subelements when filtering by genomic regions - e.g. CDSs
...
NumSubEls = 0;		// original elements may be split into subelements when filtering by genomic regions - e.g. CDSs
pParams->FiltUnderLenCnt = 0;
pParams->FiltOverLenCnt = 0;
...
while((Rslt=pCSV->NextLine()) > 0)	// onto next line containing fields
	...
	if(NumFields < 7)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Expected at least 7 fields in '%s', GetCurFields() returned '%d'",pParams->szLociFile,NumFields);
	...
	if((Rslt = FilterRegionIn(pszChrom,'+',StartLoci,EndLoci,pParams)) < 0)
		return(Rslt);
	if(Rslt == 0)
		NumFiltRegion += 1;
	else
...
if(Rslt >= 0)
	gDiagnostics.DiagOut(eDLInfo,gszProcName,"%s: Read: %d, Accepted: %d, FiltRefIDs: %d, FiltUnderLen: %d, FiltOverLen: %d, FiltRegion: %d, NumSubEls: %d",
		pParams->szLociFile,NumElsRead,NumElsAccepted,NumFiltRefIDs,pParams->FiltUnderLenCnt,pParams->FiltOverLenCnt,NumFiltRegion,NumSubEls);
...
while((CurFeatureID = pBED->GetNextFeatureID(CurFeatureID)) > 0)
	...
	FeatureLen = (EndLoci - StartLoci) + 1;
	if(FeatureLen < pParams->MinCoreLen)
		{
		pParams->FiltUnderLenCnt += 1;
		continue;
		}
	...
	if((Rslt = FilterRegionIn(szChrom,Strand,StartLoci,EndLoci,pParams)) < 0)
		return(Rslt);
	if(Rslt == 0)
		NumFiltRegion += 1;
	else
...
if(Rslt >= 0)
	gDiagnostics.DiagOut(eDLInfo,gszProcName,"%s: Read: %d, Accepted: %d, FiltRefIDs: 0, FiltUnderLen: %d, FiltOverLen: %d, FiltRegion: %d, NumSubEls: %d",
		pParams->szLociFile,NumElsRead,NumElsAccepted,pParams->FiltUnderLenCnt,pParams->FiltOverLenCnt,NumFiltRegion,NumSubEls);
return(ProcessLoci(pParams));
}
...
for(Idx = 0; Idx < pProcParams->MaxNumSpecies; Idx++)
	...
	if(pProcParams->pMEGSeqs[Idx] != NULL)
		{
		delete pProcParams->pMEGSeqs[Idx];
		pProcParams->pMEGSeqs[Idx] = NULL;
		}
	}
pProcParams->MEGLen = 0;
...
while((pCoreLoci = GetLociOverlaps(pProcParams->szRefChrom,RefChromID,'+',RefChromOfs,RefChromOfs + AlignLenNoInDels -1, pCoreLoci, pProcParams))!=NULL)
	...
	if(bInLoci)
		...
		if(EndAlignBlock(pProcParams->szRefChrom,StartLoci,pProcParams) < 0)
			return(false);
		}
	}
...
{
pProcParams->MEGBlockStart = pProcParams->MEGLen;
return(0);
}
...
{
pProcParams->MEGLen = pProcParams->MEGBlockStart;
return(0);
}

int 
EndAlignBlock(char *pszRefChrom,int StartLoci,tsProcParams *pProcParams)
{
int LineLen;
int Idx;
int Ofs;
...
int BaseCnt;
char szHeader[1024];
if(pProcParams->ProcMode == eProcModeMEG3 ||
   pProcParams->ProcMode == eProcModeFasta || 
   !pProcParams->MEGLen || 
   pProcParams->MEGBlockStart == pProcParams->MEGLen ||
   pProcParams->hOutFile <= 0)
	return(0);
...
for(Idx = 0; Idx < pProcParams->MaxNumSpecies; Idx++)
	{
	pSeq = (char *)pProcParams->pMEGSeqs[Idx];
	pDst = pSeq;
	SeqLen = 0;
	InDelCnt = 0;
	UnalignCnt = 0;
	BaseCnt = 0;
	LenRemaining = pProcParams->MEGLen;
	while(LenRemaining-- && (Base = *pSeq++))
		{
		switch(Base) {
			case '-':
				InDelCnt++;
				if(pProcParams->ProcMode == eProcModeMultiFastaSeqs)
					continue;
				break;
	...
	LineLen = sprintf(szHeader,">%s|%d|%d|%d|%d|%d|%s|%d\n",
			pProcParams->szSpecies[Idx],SeqLen,BaseCnt,InDelCnt,UnalignCnt,pProcParams->AlignBlockID,pszRefChrom,StartLoci);
	CUtility::SafeWrite(pProcParams->hOutFile,szHeader,LineLen);

	pSeq = (char *)pProcParams->pMEGSeqs[Idx];
	LenRemaining = SeqLen;
	Ofs = 0;
	while(LenRemaining) {
		LineLen = min(70,LenRemaining);
		CUtility::SafeWrite(pProcParams->hOutFile,&pSeq[Ofs],LineLen);
		CUtility::SafeWrite(pProcParams->hOutFile,"\n",1);
		Ofs += LineLen;
...
if(pProcParams->MEGLen >= (pProcParams->MaxMEGseqLen-1))
	{
	if(!pProcParams->bMEGLenTrunc)
		{
		gDiagnostics.DiagOut(eDLWarn,gszProcName,"Truncated concatenated MEG3 sequences at %d bases",pProcParams->MaxMEGseqLen-1);
		pProcParams->bMEGLenTrunc = true;
		}
	return(false);
	}
...
for(Idx = 0; Idx < pProcParams->MaxNumSpecies; Idx++)
	...
	pMEG = pProcParams->pMEGSeqs[Idx];
	pMEG[pProcParams->MEGLen] = Base;
	pMEG[pProcParams->MEGLen+1] = '\0';
	}
pProcParams->MEGLen += 1;
return(true);
}

int
WriteMEGFile(tsProcParams *pProcParams)
{
int LineLen;
int Idx;
int Ofs;
...
int BaseCnt;
char szHeader[1024];
if(pProcParams->ProcMode >= eProcModeMultiFastaAlignSeqs || !pProcParams->MEGLen || pProcParams->hOutFile <= 0)
	return(0);

if(pProcParams->ProcMode == eProcModeMEG3)
	{
	LineLen = sprintf(szHeader,"#mega\n!Title Nucleotide sequences for %s;\n!Format DataType=DNA indel=- CodeTable=Standard;\n",pProcParams->pszSpeciesList);
	CUtility::SafeWrite(pProcParams->hOutFile,szHeader,LineLen);
	LineLen = sprintf(szHeader,"!Domain=Data property=NonCoding;\n");
	CUtility::SafeWrite(pProcParams->hOutFile,szHeader,LineLen);
	}
...
for(Idx = 0; Idx < pProcParams->MaxNumSpecies; Idx++)
	...
	if(pProcParams->ProcMode == eProcModeMEG3)
		{
		LineLen = sprintf(szHeader,"\n#%s\n",pProcParams->szSpecies[Idx]);
		CUtility::SafeWrite(pProcParams->hOutFile,szHeader,LineLen);
		}
	else
		{
		...
		if(!BaseCnt)
			continue;
		LineLen = sprintf(szHeader,">%s|%d|%d|%d|%d\n",
			pProcParams->szSpecies[Idx],SeqLen,BaseCnt,InDelCnt,UnalignCnt);
		CUtility::SafeWrite(pProcParams->hOutFile,szHeader,LineLen);
		}
	pMEG = (char *)pProcParams->pMEGSeqs[Idx];
	LenRemaining = pProcParams->MEGLen;
	Ofs = 0;
	while(LenRemaining) {
		LineLen = min(70,LenRemaining);
		CUtility::SafeWrite(pProcParams->hOutFile,&pMEG[Ofs],LineLen);
		CUtility::SafeWrite(pProcParams->hOutFile,"\n",1);
		Ofs += LineLen;
		LenRemaining -= LineLen;
		}
