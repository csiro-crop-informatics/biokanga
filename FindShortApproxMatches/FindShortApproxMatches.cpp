// FindShortApproxMatches.cpp : Defines the entry point for the console application
// Processes probes against targets using hashes
// Locates matches for each probe against target with at most the specified number of mismatches
// Probes can be multifasta *.fa), bioseq (.seq), or fastq
// Target can be multifasta *.fa) or bioseq (.seq)
...
typedef struct TAG_sFSAMProbe {
	...
	int ProbeState;							// 0x01 if this probe to be skipped
...
int Process(teProcMode Mode,char *pszInputFile,char *pszProbeFile,char *pszOutputFile,char *pszChroms,bool bNestedHitsOk,bool bMisses,bool bPerChrom,char Strand,int iMaxMismatches,bool bBED,bool bBedCompat,int MinRange,int MaxRange,int MinLength, int MaxLength);
...
int ExactMatch(teProcMode Mode,int hRsltsFile,tBSFEntryID CurEntry,unsigned char *pChromSeqBuff,int ChromSeqLen,char *pszChrom, char Strand,bool bBED,bool bNestedHitsOk,int MinCounts, int MaxCounts,bool bMisses); 
...
bool bNestedHitsOk;			// ok to have same probe with exact and mismatch hits
...
struct arg_lit  *NestedHitsOk  = arg_lit0("z","nested",				"ok to report both exact and mismatch hits from same probe");

struct arg_end *end = arg_end(20);

void *argtable[] = {help,version,FileLogLevel,ScreenLogLevel,LogFile,
					Mode,InFile,OutFile,ProbesFile,NestedHitsOk,Misses,
					MaxMismatches,
...
if (!argerrors)
	...
	bNestedHitsOk = (iMaxMismatches > 0 && NestedHitsOk->count) ? true : false;
...
#endif
	gStopWatch.Start();
	Rslt = Process((teProcMode)iMode,szInputFile,szProbesFile,szOutputFile,szChromsList,bNestedHitsOk,bMisses,bPerChrom,cStrand,iMaxMismatches,bBED,bBedCompat,iMinRange,iMaxRange,iMinLength,iMaxLength);
	gStopWatch.Stop();
	Rslt = Rslt >=0 ? 0 : 1;
...
int 
Process(teProcMode Mode,char *pszInputFile,char *pszProbesFile,char *pszOutputFile,char *pszChroms,
		bool bNestedHitsOk,	// if true then accept hits with mismatches even if an exact match already located
		bool bMisses, bool bPerChrom,char Strand,int MaxMismatches,bool bBED,bool bBedCompat,int MinRange,int MaxRange,int MinLength,int MaxLength)
{
CBioSeqFile *pBioSeq;
...
while((CurEntry = pBioSeq->Next(CurEntry)) > 0)
	...
	ExactMatch(Mode,hOutFile,CurEntry,pChromSeqBuff,ChromSeqLen,szChromName,Strand,bBED,bNestedHitsOk,MinRange,MaxRange,bMisses); 
	if(MaxMismatches != 0)
...
if(Mode == ePMGlobalCnts)
	...
	gMatchCnt = 0;
	if(bMisses)
		ReportMisses(pBioSeq,hOutFile,(char *)"ALLCHROMES");
	else
		ReportCounts(hOutFile,MinRange,MaxRange,(char *)"ALLCHROMES");
	}
else
	if(Mode == ePMHitLoci &&  !bPerChrom)
		ReportMatches(pBioSeq,hOutFile,MinRange,MaxRange,bBED,bBedCompat);
...
while(pCurProbe != NULL)
	...
	if(TotHits >= MinRange && TotHits <= MaxRange)
		...
		CUtility::SafeWrite(hOutFile,szOutBuff,RsltsLen);
		}
...
do {
	...
	for(LociIdx = 0; LociIdx < pCurBlock->NumLoci; LociIdx++,pCurMatch++)
		...
		if(bBED && TotHits > 0)
			...
			RsltsLen = sprintf((char *)szRsltsBuff,"%d,\"%s\",\"%s\",\"%c\",%d,%d,\"%s\",%d\n",
...
ExactMatch(teProcMode Mode,int hRsltsFile,tBSFEntryID CurEntry,
			unsigned char *pChromSeqBuff,
			int ChromSeqLen,
			char *pszChrom, 
			char Strand,
			bool bBED,
			bool bNestedHitsOk,			// true if ok to locate approx matches even if an exact match was located
			int MinCounts, 
			int MaxCounts,
			bool bMisses) 
{
...
gDiagnostics.DiagOut(eDLInfo,gszProcName,"Exacts Hits on plus strand: %d on minus strand: %d",PlusHits,MinHits);
...
while(NumWindows--)
	...
	if((Strand == '*' || Strand == '+') && (pPutProbe = gppPlusHashes[Hash1Idx])!=NULL)
		...
		while(pPutProbe != NULL)	// iterate over all probes with same hash
			...
			pPutProbe->PrvEntryID = -1;		// no hit at current loci for this probe
			pPutProbe->PrvLociHit = -1;
			if(pPutProbe->ProbeLen <= (ChromSeqLen - Loci)  && 
					(!bMisses || !pPutProbe->TotPlusHits) &&
					!(pPutProbe->ProbeState & 0x01))
				{
				pPutProbeSeq = pPutProbe->Bases;
				pPutSeq = pLeft;
			...
			if(!(pPutProbe->PrvEntryID == CurEntry && pPutProbe->PrvLociHit == Loci) &&	// only one hit per probe per loci allowed 
				...
				if(PutSeqIdx == pPutProbe->ProbeLen)
					...
					if(Mode==ePMHitLoci)
						{
						pszAsciiBases = CSeqTrans::MapSeq2Ascii(pPutProbe->Bases,pPutProbe->ProbeLen);
						RsltsLen = sprintf((char *)szRsltsBuff,"%d,\"%s\",\"%s\",\"%c\",%u,%d,\"%s\"\n",++gMatchCnt,pPutProbe->szDescr,pszChrom,'+',Loci,pPutProbe->ProbeLen,pszAsciiBases);
						CUtility::SafeWrite(hRsltsFile,szRsltsBuff,RsltsLen);
						}
					}
				}
...
gDiagnostics.DiagOut(eDLInfo,gszProcName,"Single mismatch hits on plus strand: %d on minus strand: %d",PlusHits,MinHits);
return(PlusHits + MinHits);
}
