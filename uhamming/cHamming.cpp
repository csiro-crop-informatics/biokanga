...
while((SE2B = *pSE2++) < eBaseEOG)
	...
	// objective: if eBaseEOS anywhere within the current subsequences being compared then continue Hamming generation but
	// do not update the minimum Hamming at pHSS1/pHHS2 as appropriate dependent on which subsequences contained
	// the eBaseEOS.
	// how: increment EOS cnts if the 3' end is currently eBaseEOS, at end of this iteration EOS cnts will
	// be decremented when that eBaseEOS is detected at the 5' end
	if(SE2B == eBaseEOS)
		PPEOScnt += 1;
	if(SE1B == eBaseEOS)
		PPEOScnt += 1;
	...
	// end of current iteration so if about to lose a eBaseEOS from 5' end then can decrement EOS cnt
	if(SS1B == eBaseEOS)
		PPEOScnt -= 1;
	if(SS2B == eBaseEOS)
		PPEOScnt -= 1;
