...
if(m_p16KOfsVirtAddrs != NULL)
	...
	if(m_p16KOfsVirtAddrs != MAP_FAILED)
		munmap(m_p16KOfsVirtAddrs,(cMaxSAIRefSeqLen / 0x04000) * sizeof(UINT64));
#endif
	m_p16KOfsVirtAddrs = NULL;
	}
...
if(SAMType == eSFTBAM || SAMType == eSFTBAM_BAI)
	{
...
while((LineLen = GetNxtSAMline(szLine)) > 0)
	...
	sscanf(szLine,"%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%2048s\t",m_szParsedDescriptor, &m_ParsedFlags, m_ParsedszChrom, &m_ParsedStartLoci,&MAPQ,szCigar,szRNext,&PNext,&TLen,m_szParsedSeqBases);
...
CSAMfile::Create(eSAMFileType SAMType,	// file type, expected to be either eSFTSAM or eSFTBAM_BAI
				char *pszSAMFile,		// SAM(gz) or BAM file name
				int ComprLev,			// if BAM then BGZF compress at this requested level (0..9)
			    char *pszBAIFile,		// BAI file name - if NULL then defaults to samfile name plus '.bai' appended
				char *pszVer)			// version text to use in generated SAM/BAM headers - if NULL then defaults to cszProgVer
{
if(SAMType < eSFTSAM || SAMType > eSFTBAM_BAI || pszSAMFile == NULL || pszSAMFile[0] == '\0')
	return(eBSFerrParams);
...
if(SAMType == eSFTBAM_BAI)
	{
	if(pszBAIFile != NULL && pszBAIFile[0] != '\0')
		strcpy(m_szBAIfileName,pszBAIFile);
	else
		{
...
if(SAMType == eSFTBAM_BAI)
	...
	if(m_hOutBAIfile < 0)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Create: unable to create/truncate output file '%s'",m_szBAIfileName);
		Reset();
		return(eBSFerrCreateFile);
		}
	}
...
// if also BAI output then make initial mem allocations
if(SAMType == eSFTBAM_BAI)
	{
	...
	if((m_pBAIChunks = (tsBAIChunk *)malloc(sizeof(tsBAIChunk) * cAllocBAIChunks))==NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Create: unable to alloc memory for BAI chunks");
		Reset();
		return(eBSFerrMem);
		}
	...
	if((m_p16KOfsVirtAddrs = (UINT64 *)malloc((cMaxSAIRefSeqLen / 0x04000) * sizeof(UINT64)))==NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Create: unable to alloc memory for BAI 16kb linear virtual offsets");
		Reset();
		return(eBSFerrMem);
		}
	memset(m_p16KOfsVirtAddrs,0,(cMaxSAIRefSeqLen / 0x04000) * sizeof(UINT64));
	}
...
if(m_SAMFileType == eSFTBAM || m_SAMFileType == eSFTBAM_BAI)
	{
	if(SeqLen >= cMaxSAIRefSeqLen)
		{
		gDiagnostics.DiagOut(eDLWarn,gszProcName,"AddREfSeq: Reference sequence '%s':'%s' of length %d is longer than max allowed SAI index offset of %d",
								pszSpecies,pszSeqName,SeqLen,cMaxSAIRefSeqLen-1);
		}
	}
...
if(End >= cMaxSAIRefSeqLen)		// exceeding the limit should have been picked up earlier but better safe than sorry
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddAlignment: Unable to accept alignment (%d) in BAI index extending past 512Mbp'",End);
	Reset();
	return(eBSFerrOfs);
	}
...
Bin = BAMreg2bin(Start,End);	// which bin contains this chunk?
...
int CSAMfile::BAMreg2bin(int beg, int end)
{
--end;
...
int CSAMfile::BAMreg2bins(int beg, int end, UINT16 *plist)
{
int i = 0, k;
--end;
plist[i++] = 0;
...
if(m_SAMFileType == eSFTSAM || m_SAMFileType == eSFTSAMgz)
	...
	{
	// if read has been aligned then check that the alignment is within the limits for SAI
	if(pBAMalign->pos >= 0 && m_SAMFileType == eSFTBAM_BAI && pBAMalign->end >= cMaxSAIRefSeqLen)		// SAI can't handle alignments ending after 512Mbp!
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddAlignment: Unable to accept alignment (%d) in BAI index extending past 512Mbp'",pBAMalign->end);
		Reset();
		return(eBSFerrOfs);
		}
