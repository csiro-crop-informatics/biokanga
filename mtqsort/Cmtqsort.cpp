...
::qsort(pArgs->pArray, pArgs->NumEls, pArgs->ElSize, pArgs->CompareFunc);
//pArgs->pThis->_mtqsort(pArgs->pArray, pArgs->NumEls, pArgs->ElSize, pArgs->CompareFunc);
...
#else
    if (0 == (threadRslt = pthread_create(&threadID, NULL , _qsort_start, (void*)&ThreadArgs))) 
#endif
	return(true);
	}
ReleaseLock(true);
...
recurse:
	...
	if(!bUseThreads || !((NumEls/NumElsCurPart >= 8) && NumElsCurPart > 20000 && 
		NumElsCurPart < 100000000 &&
