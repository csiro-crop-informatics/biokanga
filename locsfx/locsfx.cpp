// locsfx.cpp : Defines the entry point for the console application.
// Processes probes against chromosome or complete assembly target
// Locates all unanchored exact matches in each probe of at least a specified minimum length and then maximally extends these matches
// left and right until specified number of mismatches
// Target expected to be a bioseq suffix array (.sfx) file, probes can be either multifasta *.fa) or bioseq (.seq)
// 
...
const unsigned int cProgVer = 312;		// increment with each release
...
const int cAllocAMDescrs = 10000;		// allocate for probe descriptors in this size increments
...
int gMaxProbeHits = cDfltProbeHits;	// maximum number of hits for any probe
...
char *gpSfxChrom;					// current suffix chromosome being processed
...
Process(etProcMode ProcMode,	// processing mode
		...
		char *pszTargFile,		// path+name of suffix array file for target sequence
...
ExactMatch(bool bAbsHitLen,		// if true then MinCoreLen is an absolute length
		   int MinCoreLen,			// otherwise it's a percentage of actual probe length
		   etRPTMasking RptMask,	// repeat mask processing mode
		   int RptThres,			// filter out matches with repeats above this percentage threshold
		   int hRsltsFile,			// write hits to this file
		   CSfxArray *pSfxArray,	// hits against sequences in this multi-suffix array
		   int CurTargEntry,		// which suffix
		   char Strand);			// '*':hits against both strands, '+' strand only, '-' strand only

int 
ApproxMatch(bool bAbsHitLen,		// if true then MinCoreLen and MinMatchLen are absolute lengths
		   int MinCoreLen,			// otherwise core length is a percentage of actual probe length
		   int MinMatchLen,			// otherwise minimum match length is a percentage of actual probe length
	       int MaxMismatches,			// hits to have at most this many mismatches in the left or right flank around exactly matching core
		   etRPTMasking RptMask,	// repeat mask processing mode
		   int RptThres,			// filter out matches with repeats above this percentage threshold
			int hRsltsFile,			// write hits to this file
...
int FilterApproxMatches(sAMProbe *pProbe,	// probe
...
int FilterExactMatches(sAMProbe *pProbe,// probe
...
int iInType;				// input file type: 0=multifasta, 1=bioseq
...
struct arg_file *TargFile = arg_file1("i","inindex","<file>",		"input from target bioseq suffix array (.sfx) file");
struct arg_int *InType=arg_int0("T", "intype",		"<int>",        "input probe file type: 0=multifasta, 1=bioseq");
struct arg_file *ProbesFile = arg_file1("I","inprobes","<file>",	"input from multifasta probe or bioseq file");
struct arg_file *RsltsFile = arg_file1("o","outmatches","<file>",	"output matches to this file as CSV");

struct arg_int *MaxProbeHits=arg_int0("p", "MaxProbeHits","<int>",	"Maximum number of probe loci hits to report (1..1000000, default is 1000)");
...
void *argtable[] = {help,version,FileLogLevel,ScreenLogLevel,LogFile,
					ProcMode,InType,TargFile,RsltsFile,ProbesFile,MaxProbeHits,AbsHitLen,MinCoreLen,MinMatchLen,MaxMismatches,RptMask,RptThres,Strand,
...
if (!argerrors)
	...
	iInType = InType->count ? InType->ival[0] : 0;
	if(iInType < 0)
		iInType = 0;
	else
		if(iInType > 1)
	...
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Target bioseq suffix array file: %s",szTargFile);
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Target species: '%s'",szTargSpecies);
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Probes from %s file: %s",iInType > 0 ? "bioseq" : "multifasta",szProbesFile);
...
#endif
	gStopWatch.Start();
	Rslt = Process(eMode,iMaxProbeHits,bAbsHitLen,iMinCoreLen,iMinMatchLen,iMaxMismatches,(etFileType)iInType,szProbesFile,szTargFile,szRsltsFile,szProbeSpecies,szTargSpecies,cStrand,(etRPTMasking)iRptMask,iRptThres);
	gStopWatch.Stop();
	Rslt = Rslt >=0 ? 0 : 1;
...
Process(etProcMode ProcMode,	// processing mode
	   ...
	   int RptThres)
{
CSfxArray *pSfxArray = NULL;
CFasta *pFasta = NULL;
CBioSeqFile *pBioSeqFile = NULL;

unsigned char szSfxSource[cBSFSourceSize];
unsigned char szDescription[cBSFDescriptionSize];
tBSFEntryID CurTargEntry;				// current suffix array entry being processed
...
// open bioseq file containing suffix arrays
...
if((Rslt=pSfxArray->Open(pszTargFile,false))!=eBSFSuccess)
	{
	while(pSfxArray->NumErrMsgs())
		gDiagnostics.DiagOut(eDLFatal,gszProcName,pSfxArray->GetErrMsg());
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"Unable to open input bioseq suffix array file '%s'",pszTargFile);
	DeleteDescrs(&gProbeDescrs);
	DeleteProbeFile();
	delete pSfxArray;
	return(Rslt);
...
// iterate over each target entry (chromosome) in suffix file
// and find target hits against the probes
...
while((CurTargEntry = pSfxArray->Next(CurTargEntry)) > 0)
	{
		// get entry name - assume it will be a chromosome/contig name
	pSfxArray->GetNameDescription(CurTargEntry,cBSFSourceSize-1,(char *)&szSfxSource,
									cBSFDescriptionSize-1,(char *)&szDescription);
	gpSfxChrom = (char *)szSfxSource;

	gDiagnostics.DiagOut(eDLInfo,gszProcName,"Processing Target Sfx: %s %s|%s",pszTargSpecies,szSfxSource,szDescription);

	if(MaxMismatches == 0)			// if hits must be exact with no mismatches allowed
		Rslt = ExactMatch(bAbsHitLen,MinCoreLen,RptMask,RptThres,ghRsltsFile,pSfxArray,CurTargEntry,Strand);
		...
		Rslt = ApproxMatch(bAbsHitLen,
					...
					pSfxArray,					// hits against sequences in this multi-suffix array
					CurTargEntry,				// which suffix
...
if(NumHits < 0 || ProbeID != pProbe->ProbID || TargID != TargetEntryID) 
	{
	ProbeID = pProbe->ProbID;
	TargID = TargetEntryID;
	NumHits = 1;
	return(1);
	}
if(NumHits++ > (gMaxProbeHits * 500))	// this is simply to ensure that if a low limit was set then we don't bother continueing with probe when it will be filtered out later
	return(0);
return(1);			
}
...
if(gProcMode == ePMHitLoci)
	...
	Len = sprintf(szLineBuff,"%d,\"%s\",\"%s\",\"%s\",%d,%d,%d,\"%s\",0,\"%c\",%d,\"%s\",%d,%d\n",
			++CoreID,"exactcore",gpszTargSpecies,gpSfxChrom,
...
for(ProbeIdx = 0; ProbeIdx < gProbeDescrs.NumDescrs; ProbeIdx++, pCurDescr++)
	{
	if(!(pCurDescr->ProbID % 10))
		printf("\rProbe %d '%s'",pCurDescr->ProbID,pCurDescr->szDescr);
	
	// no point in continuing with this probe if already had more hits than max requested
	if(((pCurDescr->TotMinHits + pCurDescr->TotPlusHits) > gMaxProbeHits))
		continue;
	...
	if(Strand == '*' || Strand == '+')
		...
		if((Rslt = pSfxArray->LocateExacts(pCurDescr,pCurDescr->ProbeLen,pProbeBases,MinHitLen,FilterExactMatches,HandleExactMatches,CurTargEntry,RptMask,RptThres))>=0)
			gMatchCnt += Rslt;
		else
			return(Rslt);
		}
	...
	if(Strand == '*' || Strand == '-')
		...
		if(((pCurDescr->TotMinHits + pCurDescr->TotPlusHits) > gMaxProbeHits))
			continue;
		Rslt = pSfxArray->LocateExacts(pCurDescr,pCurDescr->ProbeLen,pProbeBases,MinHitLen,FilterExactMatches,HandleExactMatches,CurTargEntry,RptMask,RptThres);
		if(Rslt >= 0)
			gMatchCnt += Rslt;
		else
			return(Rslt);
...
for(ProbeIdx = 0; ProbeIdx < gProbeDescrs.NumDescrs; ProbeIdx++, pCurDescr++)
	...
	if(Strand == '*' || Strand == '+')
		...
		if((Rslt = pSfxArray->LocateNearExacts(pCurDescr,
							...
							HandleApproxMatches,	// handleer for accepted matches
							CurTargEntry,
							RptMask,
