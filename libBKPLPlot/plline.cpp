...
c_plpath( PLINT n, PLFLT x1, PLFLT y1, PLFLT x2, PLFLT y2 )
{
    ...
    if ( m_plsc->coordinate_transform == NULL )
    {
...
c_plstyl( PLINT nms, const PLINT *mark, const PLINT *space )
{
    ...
    for ( i = 0; i < nms; i++ )
    {
        m_plsc->mark[i]  = mark[i];
        m_plsc->space[i] = space[i];
    }

    m_plsc->curel   = 0;
    m_plsc->pendn   = 1;
    m_plsc->timecnt = 0;
    m_plsc->alarm   = nms > 0 ? mark[0] : 0;
}
...
{
    xline[0] = m_plsc->currx;
    xline[1] = x;
    yline[0] = m_plsc->curry;
    yline[1] = y;
...
plP_movwor( PLFLT x, PLFLT y )
{
    ...
    m_plsc->currx = plP_wcpcx( xt );
    m_plsc->curry = plP_wcpcy( yt );
}
...
pllclp( PLINT *x, PLINT *y, PLINT npts )
{
    plP_pllclp( x, y, npts, m_plsc->clpxmi, m_plsc->clpxma,
        m_plsc->clpymi, m_plsc->clpyma, genlin );
}
...
// Handle remaining polyline
    ...
    m_plsc->currx = x[npts - 1];
    m_plsc->curry = y[npts - 1];
...
// Compute the timer step
    ...
    nxstep = nxp * m_plsc->umx;
    nystep = nyp * m_plsc->umy;
    ...
    while ( i < modulo )
    {
        pix_distance = ( m_plsc->alarm - m_plsc->timecnt + tstep - 1 ) / tstep;
        i           += pix_distance;
        if ( i > modulo )
            pix_distance -= ( i - modulo );
        m_plsc->timecnt += pix_distance * tstep;
...
// Update line style variables when alarm goes off
        ...
        while ( m_plsc->timecnt >= m_plsc->alarm )
        {
            if ( m_plsc->pendn != 0 )
            {
                m_plsc->pendn    = 0;
                m_plsc->timecnt -= m_plsc->alarm;
                m_plsc->alarm    = m_plsc->space[m_plsc->curel];
            }
            else
            {
                m_plsc->pendn    = 1;
                m_plsc->timecnt -= m_plsc->alarm;
                m_plsc->curel++;
                if ( m_plsc->curel >= m_plsc->nms )
                    m_plsc->curel = 0;
                m_plsc->alarm = m_plsc->mark[m_plsc->curel];
            }
        }
        lastx = xtmp;
        lasty = ytmp;
