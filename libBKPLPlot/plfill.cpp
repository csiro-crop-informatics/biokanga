...
// Loop over sets of lines in pattern
    ...
    for ( k = 0; k < m_plsc->nps; k++ )
    {
        bufferleng = 0;

        temp = DTOR * m_plsc->inclin[k] * 0.1;
        si   = sin( temp ) * m_plsc->ypmm;
        ci   = cos( temp ) * m_plsc->xpmm;
        ...
        dinc = (PLINT) ( m_plsc->delta[k] * SSQR( m_plsc->ypmm * ABS( ci ),
                             m_plsc->xpmm * ABS( si ) ) / 1000. );
