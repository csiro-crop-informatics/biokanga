...
static void pl_drawcontlabel( PLFLT tpx, PLFLT tpy, char *flabel, PLFLT *distance, PLINT *lastindex )
{
    ...
    delta_x = plP_pcdcx( m_plsc->currx ) - plP_pcdcx( plP_wcpcx( tpx ) );
    delta_y = plP_pcdcy( m_plsc->curry ) - plP_pcdcy( plP_wcpcy( tpy ) );

    currx_old = m_plsc->currx;
    curry_old = m_plsc->curry;
    ...
    *distance += sqrt( delta_x * delta_x + delta_y * delta_y );
    ...
    if ( (int) ( fabs( *distance / contlabel_space ) ) > *lastindex )
    {
        ...
        mx = (double) m_plsc->wpxscl / (double) m_plsc->phyxlen;
        my = (double) m_plsc->wpyscl / (double) m_plsc->phyylen;
...
plP_pcwcx( PLINT x )
{
    return ( ( x - m_plsc->wpxoff ) / m_plsc->wpxscl );
}
...
plP_pcwcy( PLINT y )
{
    return ( ( y - m_plsc->wpyoff ) / m_plsc->wpyscl );
}
