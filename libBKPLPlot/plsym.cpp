...
//
// Modified by Stuart Stephen, March 2014, enabling loading of embedded fonts
// Contains the standard and extended font information for the standard fonts distributed with PLPlot as embedded 
// initialised structures in "plstnd5.fnt.h" and  "plxtnd5.fnt.h" 
// Saves having to install the original font files and configuring the plotting application for location of these font files
// Note that processing firstly looks for the original font file (allows users to modify these to suit application requirements) and
// only if the specific font file can't be loaded will then attempt to load the embedded font.
...
#include "plxtnd5.fnt.h"			// embedded 
#include "plstnd5.fnt.h"
...
// moved to plstr.h, m_plsc->cfont  static PLINT font = 1;  current font
...
{
    PLINT i, sym, ifont = m_plsc->cfont;
    PLFLT xt, yt;
...
plhrsh( PLINT ch, PLINT x, PLINT y )
{
    ...
    // symbols.
    //
    if ( ( m_plsc->dev_text ) && ( m_plsc->dev_unicode ) && ( !m_plsc->dev_hrshsym ) )
    {
        ...
        else
        {
            ...
            m_plsc->original_chrht  = m_plsc->chrht;
            m_plsc->original_chrdef = m_plsc->chrdef;
            m_plsc->chrht           = m_plsc->symht;
            m_plsc->chrdef          = m_plsc->symdef;
            ...
            m_plsc->chrht  = m_plsc->original_chrht;
            m_plsc->chrdef = m_plsc->original_chrdef;
        }
    }
    else
    {
...
// Compute how many physical pixels correspond to a character pixel
    ...
    xscale = scale * m_plsc->xpmm;
    yscale = scale * m_plsc->ypmm;
...
// Open clip limits to subpage limits
    ...
    {
        vert  = 1;
        xdv   = m_plsc->vpdxmi + ( m_plsc->vpdxma - m_plsc->vpdxmi ) * pos;
        ydv   = m_plsc->vpdymi;
        dispx = 0;
        dispy = -disp;
    }
    ...
    else if ( plP_stindex( side, "LV" ) != -1 || plP_stindex( side, "lv" ) != -1 )
    {
        vert  = 0;
        xdv   = m_plsc->vpdxmi;
        ydv   = m_plsc->vpdymi + ( m_plsc->vpdyma - m_plsc->vpdymi ) * pos;
        dispx = -disp;
        dispy = 0;
    }
...
c_plptex( PLFLT wx, PLFLT wy, PLFLT dx, PLFLT dy, PLFLT just, const char *text )
{
    ...
    cc   = m_plsc->wmxscl * dxt;
    ss   = m_plsc->wmyscl * dyt;
        ...
        else
        {
            if ( plcvec( ch, &vxygrid ) )
                plchar( vxygrid, xform, base, oline, uline, refx, refy, scale,
                    m_plsc->xpmm, m_plsc->ypmm, &xorg, &yorg, &width );
        }
    }
    m_plsc->nms = style;
}
...
plstrl( const char *string )
{
    ...
    if ( m_plsc->has_string_length )
    {
        m_plsc->get_string_length = 1;
        c_plmtex( "t", 0.0, 0.0, 0.0, string );
        m_plsc->get_string_length = 0;
        return (PLFLT) m_plsc->string_length;
    }
...
{
    PLINT     ch, ifont = m_plsc->cfont, ig, j = 0, lentxt = (PLINT) strlen( text );
    char      test, esc;
...
plfntld( PLINT fnt )
{
    ...
    plfontrel();
    fontloaded = 1;				// big assumption!
    charset    = fnt;

    if ( fnt )
        pdfs = plLibOpenPdfstrm( PL_XFONT );		// allow font to be loaded from file so users can override the embedded fonts
    else
    ...
    if ( pdfs == NULL )
		{
		bUseEmbedded = true;
		pEmbeddedFont = fnt == 0 ? plstnd5_fnt : plxtnd5_fnt;
		numberchars = *pEmbeddedFont++;	
		numberfonts = *pEmbeddedFont++;
		}
	else
		{
		bUseEmbedded = false;
// Read fntlkup[]
	...
	if(bUseEmbedded)
		{
		memcpy(fntlkup,pEmbeddedFont,sizeof(short unsigned int) * bffrleng);
		pEmbeddedFont += sizeof(short unsigned int) * bffrleng;
		indxleng = *(unsigned short int *)pEmbeddedFont;
		pEmbeddedFont += sizeof(short unsigned int);
		}
	else
		{
...
// Read fntindx[]
	...
	if(bUseEmbedded)
		{
		memcpy(fntindx,pEmbeddedFont,sizeof(short unsigned int) * indxleng);
		pEmbeddedFont += sizeof(short unsigned int) * indxleng;
		bffrleng = *(unsigned short int *)pEmbeddedFont;
		pEmbeddedFont += sizeof(short unsigned int);
		}
	else
		{
...
// Since this is an array of char, there are no endian problems
	...
	if(bUseEmbedded)
		memcpy(fntbffr,pEmbeddedFont,sizeof(short unsigned int) * bffrleng);
	else
