// $Id: plargs.c 12677 2013-11-12 00:54:54Z airwin $
//
...
// If program name is first argument, save and advance
    ...
    if ( !mode_noprogram )
    {
        m_plsc->program = plstrdup( argv[0] );
        program       = (const char *) m_plsc->program;
        --myargc; ++argv;
    }
    if ( myargc == 0 )
        return 0;
...
//! Set portrait mode.  If m_plsc->portrait = 1, then the orientation for certain
...
{
    m_plsc->server_name = plstrdup( opt_arg );
    return 0;
}
...
{
    m_plsc->plserver = plstrdup( opt_arg );
    return 0;
}
...
{
    if ( ( m_plsc->plwindow = (char *) malloc( (size_t) ( 1 + strlen( opt_arg ) ) * sizeof ( char ) ) ) == NULL )
    {
        plexit( "opt_plwindow: Insufficient memory" );
    }
    strcpy( m_plsc->plwindow, opt_arg );
    return 0;
}
...
{
    m_plsc->auto_path = plstrdup( opt_arg );
    return 0;
}
...
// The TK driver uses the geometry string directly
    ...
    if ( ( m_plsc->geometry = (char *) malloc( (size_t) ( 1 + strlen( opt_arg ) ) * sizeof ( char ) ) ) == NULL )
    {
        plexit( "opt_geo: Insufficient memory" );
    }

    strcpy( m_plsc->geometry, opt_arg );
...
{
    if ( ( m_plsc->tk_file = (char *) malloc( (size_t) ( 1 + strlen( opt_arg ) ) * sizeof ( char ) ) ) == NULL )
    {
        plexit( "opt_tk_file: Insufficient memory" );
    }

    strcpy( m_plsc->tk_file, opt_arg );
    return 0;
}
