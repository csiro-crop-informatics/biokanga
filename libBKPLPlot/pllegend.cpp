// $Id: pllegend.c 12873 2013-12-17 01:27:14Z airwin $
...
static void plgvpsp( PLFLT *p_xmin, PLFLT *p_xmax, PLFLT *p_ymin, PLFLT *p_ymax )
{
    ...
    if ( ( m_plsc->cursub <= 0 ) || ( m_plsc->cursub > ( m_plsc->nsubx * m_plsc->nsuby ) ) )
    {
        plabort( "plgvpsp: Please call pladv or plenv to go to a subpage" );
        return;
    }
    *p_xmin = ( m_plsc->vpdxmi - m_plsc->spdxmi ) / ( m_plsc->spdxma - m_plsc->spdxmi );
    *p_xmax = ( m_plsc->vpdxma - m_plsc->spdxmi ) / ( m_plsc->spdxma - m_plsc->spdxmi );
    *p_ymin = ( m_plsc->vpdymi - m_plsc->spdymi ) / ( m_plsc->spdyma - m_plsc->spdymi );
    *p_ymax = ( m_plsc->vpdyma - m_plsc->spdymi ) / ( m_plsc->spdyma - m_plsc->spdymi );
}
...
static PLFLT get_character_or_symbol_height( PLBOOL ifcharacter )
{
    ...
    {
        default_mm     = m_plsc->symdef;
        char_height_mm = m_plsc->symht;
    }
...

{
    ...
    PLINT col0_save       = m_plsc->icol0,
          line_style_save = m_plsc->line_style,
          pattern_save    = m_plsc->patt;
    PLFLT line_width_save = m_plsc->width;
    PLFLT text_scale_save = m_plsc->chrht / m_plsc->chrdef;
...
#define max_opts    25
    ...
    //PLFLT aspspp = ( ( m_plsc->sppxma - m_plsc->sppxmi ) / m_plsc->xpmm ) /
    //               ( ( m_plsc->sppyma - m_plsc->sppymi ) / m_plsc->ypmm );
    ...
    // x and y.
    //PLFLT spxpmm          = m_plsc->xpmm / ( m_plsc->sppxma - m_plsc->sppxmi );
    //PLFLT spypmm          = m_plsc->ypmm / ( m_plsc->sppyma - m_plsc->sppymi );
    ...
    if ( if_bb )
    {
        ...
        m_plsc->boxbb_xmin = m_plsc->vppxmi / m_plsc->xpmm;
        m_plsc->boxbb_xmax = m_plsc->vppxma / m_plsc->xpmm;
        m_plsc->boxbb_ymin = m_plsc->vppymi / m_plsc->ypmm;
        m_plsc->boxbb_ymax = m_plsc->vppyma / m_plsc->ypmm;
    ...
    if ( opt & PL_COLORBAR_LABEL_LEFT )
    {
        if ( opt & PL_COLORBAR_ORIENT_TOP || opt & PL_COLORBAR_ORIENT_BOTTOM )
        {
            ...
            if ( if_bb )
            {
                m_plsc->boxbb_xmin = MIN( m_plsc->boxbb_xmin, m_plsc->vppxmi /
                    m_plsc->xpmm - parallel_height_mm );
                m_plsc->boxbb_ymin = MIN( m_plsc->boxbb_ymin,
                    0.5 * ( m_plsc->vppymi + m_plsc->vppyma ) /
                    m_plsc->ypmm - 0.5 * label_length_mm );
                m_plsc->boxbb_ymax = MAX( m_plsc->boxbb_ymax,
                    0.5 * ( m_plsc->vppymi + m_plsc->vppyma ) /
                    m_plsc->ypmm + 0.5 * label_length_mm );
            }
        }
        else
        {
            ...
            {
                m_plsc->boxbb_xmin = MIN( m_plsc->boxbb_xmin, m_plsc->vppxmi /
                    m_plsc->xpmm - perpendicular_height_mm - label_length_mm );
            }
        }
    ...
    else if ( opt & PL_COLORBAR_LABEL_RIGHT )
    {
        if ( opt & PL_COLORBAR_ORIENT_TOP || opt & PL_COLORBAR_ORIENT_BOTTOM )
        {
            ...
            if ( if_bb )
            {
                m_plsc->boxbb_xmax = MAX( m_plsc->boxbb_xmax, m_plsc->vppxma /
                    m_plsc->xpmm + parallel_height_mm );
                m_plsc->boxbb_ymin = MIN( m_plsc->boxbb_ymin,
                    0.5 * ( m_plsc->vppymi + m_plsc->vppyma ) /
                    m_plsc->ypmm - 0.5 * label_length_mm );
    ...
    else if ( opt & PL_COLORBAR_LABEL_TOP )
    {
        ...
        if ( if_bb )
        {
            m_plsc->boxbb_ymax = MAX( m_plsc->boxbb_ymax, m_plsc->vppyma /
                m_plsc->ypmm + parallel_height_mm );
            m_plsc->boxbb_xmin = MIN( m_plsc->boxbb_xmin,
                0.5 * ( m_plsc->vppxmi + m_plsc->vppxma ) /
                m_plsc->xpmm - 0.5 * label_length_mm );
            m_plsc->boxbb_xmax = MAX( m_plsc->boxbb_xmax,
                0.5 * ( m_plsc->vppxmi + m_plsc->vppxma ) /
                m_plsc->xpmm + 0.5 * label_length_mm );
        }
    }
    else if ( opt & PL_COLORBAR_LABEL_BOTTOM )
    {
        ...
        if ( if_bb )
        {
            m_plsc->boxbb_ymin = MIN( m_plsc->boxbb_ymin, m_plsc->vppymi /
                m_plsc->ypmm - parallel_height_mm );
            m_plsc->boxbb_xmin = MIN( m_plsc->boxbb_xmin,
                0.5 * ( m_plsc->vppxmi + m_plsc->vppxma ) /
                m_plsc->xpmm - 0.5 * label_length_mm );
                  ...
                  )
{
    ...
    *p_colorbar_width_bb  = ( m_plsc->boxbb_xmax - m_plsc->boxbb_xmin ) * spxpmm;
    *p_colorbar_height_bb = ( m_plsc->boxbb_ymax - m_plsc->boxbb_ymin ) * spypmm;
    ...
    *p_dx_subpage = -m_plsc->boxbb_xmin * spxpmm;
    *p_dy_subpage = prior_bb_height - m_plsc->boxbb_ymax * spypmm;
              ...
              const PLINT *n_values, const PLFLT * const *values )
{
    ...
    for ( i = n_axes - 1; i >= 0; i-- )
    {
        ...
        if ( i == n_axes - 1 )
        {
            ...
            {
                if ( opt & PL_COLORBAR_ORIENT_RIGHT )
                    m_plsc->boxbb_xmin = MIN( m_plsc->boxbb_xmin, -cap_extent_mm );
                if ( opt & PL_COLORBAR_ORIENT_TOP )
                    m_plsc->boxbb_ymin = MIN( m_plsc->boxbb_ymin, -cap_extent_mm );
                if ( opt & PL_COLORBAR_ORIENT_LEFT )
                    m_plsc->boxbb_xmax = MAX( m_plsc->boxbb_xmax, colorbar_width_mm + cap_extent_mm );
                if ( opt & PL_COLORBAR_ORIENT_BOTTOM )
                    m_plsc->boxbb_ymax = MAX( m_plsc->boxbb_ymax, colorbar_height_mm + cap_extent_mm );
            }
