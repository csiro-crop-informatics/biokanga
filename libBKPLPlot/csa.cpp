...
static void triangle_addpoint( triangle* t, point* p )
{
    if ( t->nallocated == t->npoints )
    {
        if ( t->nallocated == 0 )
        {
            t->points     = (point **)malloc( NPASTART * sizeof ( point* ) );
            t->nallocated = NPASTART;
        }
        else
        {
            t->points      = (point **)realloc( t->points, (size_t) ( t->nallocated * 2 ) * sizeof ( point* ) );
            t->nallocated *= 2;
        }
    }
...
static void square_addpoint( square* s, point* p )
{
    if ( s->nallocated == s->npoints )
    {
        if ( s->nallocated == 0 )
        {
            s->points     = (point **)malloc( NPASTART * sizeof ( point* ) );
            s->nallocated = NPASTART;
        }
        else
        {
            s->points      = (point **)realloc( s->points, (size_t) ( s->nallocated * 2 ) * sizeof ( point* ) );
            s->nallocated *= 2;
        }
    }
...
csa* csa_create()
{
    ...
    a->points     = (point **)malloc( NPASTART * sizeof ( point* ) );
...
void csa_addpoints( csa* a, int n, point points[] )
{
    ...
    if ( na != a->nallocated )
    {
        a->points     = (point **)realloc( a->points, (size_t) na * sizeof ( point* ) );
        a->nallocated = na;
    }
...
static void csa_squarize( csa* a )
{
    ...
    //
    a->pt = (triangle **)malloc( (size_t) ( ( a->ni / 2 + 1 ) * a->nj ) * sizeof ( triangle* ) );
...
static void getsquares( csa* a, triangle* t, int* n, square*** squares )
{
    ...
    *n           = 0;
    ( *squares ) = (square **)malloc( (size_t) ( ( imax - imin + 1 ) * ( jmax - jmin + 1 ) ) * sizeof ( square* ) );
...
{
    double** V = (double **)alloc2d( ni, ni, sizeof ( double ) );
    double** B = (double **)alloc2d( nj, ni, sizeof ( double ) );
    int   i, j, ii;
...
static void csa_findprimarycoeffs( csa* a )
{
    ...
    for ( i = 0; i < a->npt; ++i )
    {
        ...
        double  * z       = (double *)malloc( (size_t) npoints * sizeof ( double ) );
        int     q         = n2q( t->npoints );
        int     ok        = 1;
        double  b[10];
        double  b1[6];
