// $Id: plbox.c 12950 2014-01-22 20:22:02Z airwin $
//
...
// Open the clip limits to the subpage limits
    ...
    vppxmi = m_plsc->vppxmi;
    vppxma = m_plsc->vppxma;
    vppymi = m_plsc->vppymi;
    vppyma = m_plsc->vppyma;
...
// Tick and subtick sizes in device coords
    ...
    xmajor = MAX( ROUND( m_plsc->majht * m_plsc->ypmm ), 1 );
    ymajor = MAX( ROUND( m_plsc->majht * m_plsc->xpmm ), 1 );
    xminor = MAX( ROUND( m_plsc->minht * m_plsc->ypmm ), 1 );
    yminor = MAX( ROUND( m_plsc->minht * m_plsc->xpmm ), 1 );
...
// Set up tick variables
    ...
    if ( m_plsc->if_boxbb )
    {
        ...
        if ( lix && ( lbx || lux ) && ( ltx && !lxx ) )
            m_plsc->boxbb_ymin -= xmajor / m_plsc->ypmm;
        if ( liy && ( lcy || lwy ) && ( lty && !lxy ) )
            m_plsc->boxbb_xmax += ymajor / m_plsc->xpmm;
        if ( lix && ( lcx || lwx ) && ( ltx && !lxx ) )
            m_plsc->boxbb_ymax += xmajor / m_plsc->ypmm;
        if ( liy && ( lby || luy ) && ( lty && !lxy ) )
            m_plsc->boxbb_xmin -= ymajor / m_plsc->xpmm;
    }
    else
    {
...
// grid so store this stuff.
    ...
    *zbflg = plP_stsearch( zopt, 'd' );
    if ( *zbflg )
    {
        ...
        *zbcol   = m_plsc->icol0; // and color
        *zbwidth = m_plsc->width; // and line width
    }
...
// Tick and subtick sizes in device coords
    ...
    major = MAX( ROUND( m_plsc->majht * m_plsc->ypmm ), 1 );
    minor = MAX( ROUND( m_plsc->minht * m_plsc->ypmm ), 1 );
        ...
        PLFLT disp, PLFLT pos, PLFLT just, const char *text )
{
    ...
    cc   = m_plsc->wmxscl * ( wx2 - wx1 );
    ss   = m_plsc->wmyscl * ( wy2 - wy1 );
...
// Tick and subtick sizes in device coords
    ...
    dxm  = dx * m_plsc->wmxscl;
    dym  = dy * m_plsc->wmyscl;
...
label_box( const char *xopt, PLFLT xtick1, const char *yopt, PLFLT ytick1 )
{
    ...
    PLBOOL custom_exponent_placement = !m_plsc->label_func && m_plsc->label_data;
...
// similarly for vpwymi
    ...
    if ( ( lmx || lnx ) && ( ltx || lxx ) )
    {
        ...
        for ( tn = tp; BETW( tn, vpwxmi, vpwxma ); tn += xtick1 )
        {
            if ( ldx )
            {
                strfqsas( string, STRING_LEN, timefmt, (double) tn, m_plsc->qsasconfig );
            }
            else
            {
            ...
            if ( m_plsc->if_boxbb )
            {
                ...
                pos_mm           = ( m_plsc->vppxmi + pos *
                                     ( m_plsc->vppxma - m_plsc->vppxmi ) ) /
                                   m_plsc->xpmm;
            }
            if ( lnx )
            {
        ...
        if ( !llx && !ldx && !lox && xmode )
        {
            ...
            {
                height = ( (PLLabelDefaults *) m_plsc->label_data )->exp_label_disp;
                pos    = ( (PLLabelDefaults *) m_plsc->label_data )->exp_label_pos;
                just   = ( (PLLabelDefaults *) m_plsc->label_data )->exp_label_just;
            }
            else
            {
                height = 3.2;
                ...
                if ( lvy )
                {
                    ...
                    if ( m_plsc->if_boxbb )
                    {
                        ...
                        m_plsc->boxbb_xmin = MIN( m_plsc->boxbb_xmin, m_plsc->vppxmi /
                            m_plsc->xpmm - height_mm - string_length_mm );
                        pos_mm = ( m_plsc->vppymi + pos *
                                   ( m_plsc->vppyma - m_plsc->vppymi ) ) /
                                 m_plsc->ypmm;
            ...
            if ( lmy )
            {
                ...
                if ( m_plsc->if_boxbb )
                {
                    ...
                    pos_mm           = ( m_plsc->vppxmi + pos *
                                         ( m_plsc->vppxma - m_plsc->vppxmi ) ) /
                                       m_plsc->xpmm;
                    m_plsc->boxbb_xmax = MAX( m_plsc->boxbb_xmin,
                        pos_mm + string_length_mm );
                }
                else
                {
...
// similarly for vpwymi
    ...
    if ( m_plsc->if_boxbb )
    {
        PLINT xmajor = MAX( ROUND( m_plsc->majht * m_plsc->ypmm ), 1 );
        PLINT ymajor = MAX( ROUND( m_plsc->majht * m_plsc->xpmm ), 1 );
        // Bounding-box limits for the box in mm before corrections
        // for decorations are applied.
        m_plsc->boxbb_xmin = m_plsc->vppxmi / m_plsc->xpmm;
        m_plsc->boxbb_xmax = m_plsc->vppxma / m_plsc->xpmm;
...
plform( PLINT axis, PLFLT value, PLINT scale, PLINT prec, char *string, PLINT len, PLBOOL ll, PLBOOL lf, PLBOOL lo )
{
    ...
    // use default.
    if ( lo && m_plsc->label_func )
    {
        ( *m_plsc->label_func )( axis, value, string, len, m_plsc->label_data );
    }
    else
    {
        if ( lo )
...
{
    m_plsc->label_func = label_func;
    m_plsc->label_data = label_data;
}
...
plgesc_string( void )
{
    ...
    // if m_plsc->esc is in its initial state.
    if ( m_plsc->esc == '\0' )
        m_plsc->esc = '#';
