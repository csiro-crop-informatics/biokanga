          ...
          PLINT just, PLINT axis, PLINT old )
{
    ...
    if ( m_plsc->nsubx * m_plsc->nsuby == 1 ) // not multiplot mode
        old = 1;
...
//  coordinates
    ...
    xmin = plP_dcscx( plP_mmdcx( (PLFLT) ( plP_dcmmx( m_plsc->spdxmi ) + lb ) ) );
    xmax = plP_dcscx( plP_mmdcx( (PLFLT) ( plP_dcmmx( m_plsc->spdxma ) - rb ) ) );
    ymin = plP_dcscy( plP_mmdcy( (PLFLT) ( plP_dcmmy( m_plsc->spdymi ) + tb ) ) );
    ymax = plP_dcscy( plP_mmdcy( (PLFLT) ( plP_dcmmy( m_plsc->spdyma ) - bb ) ) );
...
//
// Defines a "standard" viewport with seven character heights for
// the left margin, 15 for bottom, and four character heights everywhere else.
// Added by stuart.stephen@csiro.au
//--------------------------------------------------------------------------
...
//  coordinates
    ...
    lb = LefttMargin * m_plsc->chrht;
    rb = RightMargin * m_plsc->chrht;
    tb = TopMargin * m_plsc->chrht;
    bb = BottomMargin * m_plsc->chrht;
...
c_plvpor( PLFLT xmin, PLFLT xmax, PLFLT ymin, PLFLT ymax )
{
    ...
    m_plsc->vpdxmi = m_plsc->spdxmi + ( m_plsc->spdxma - m_plsc->spdxmi ) * xmin;
    m_plsc->vpdxma = m_plsc->spdxmi + ( m_plsc->spdxma - m_plsc->spdxmi ) * xmax;
    m_plsc->vpdymi = m_plsc->spdymi + ( m_plsc->spdyma - m_plsc->spdymi ) * ymin;
    m_plsc->vpdyma = m_plsc->spdymi + ( m_plsc->spdyma - m_plsc->spdymi ) * ymax;

    m_plsc->vppxmi = plP_dcpcx( m_plsc->vpdxmi );
    m_plsc->vppxma = plP_dcpcx( m_plsc->vpdxma );
    m_plsc->vppymi = plP_dcpcy( m_plsc->vpdymi );
    m_plsc->vppyma = plP_dcpcy( m_plsc->vpdyma );

    m_plsc->clpxmi = MAX( m_plsc->vppxmi, m_plsc->phyxmi );
    m_plsc->clpxma = MIN( m_plsc->vppxma, m_plsc->phyxma );
    m_plsc->clpymi = MAX( m_plsc->vppymi, m_plsc->phyymi );
    m_plsc->clpyma = MIN( m_plsc->vppyma, m_plsc->phyyma );
...
c_plsvpa( PLFLT xmin, PLFLT xmax, PLFLT ymin, PLFLT ymax )
{
    ...
    sxmin = plP_dcmmx( m_plsc->spdxmi );
    symin = plP_dcmmy( m_plsc->spdymi );

    m_plsc->vpdxmi = plP_mmdcx( (PLFLT) ( sxmin + xmin ) );
    m_plsc->vpdxma = plP_mmdcx( (PLFLT) ( sxmin + xmax ) );
    m_plsc->vpdymi = plP_mmdcy( (PLFLT) ( symin + ymin ) );
    m_plsc->vpdyma = plP_mmdcy( (PLFLT) ( symin + ymax ) );
    ...
    m_plsc->clpxmi = plP_dcpcx( m_plsc->vpdxmi );
    m_plsc->clpxma = plP_dcpcx( m_plsc->vpdxma );
    m_plsc->clpymi = plP_dcpcy( m_plsc->vpdymi );
    m_plsc->clpyma = plP_dcpcy( m_plsc->vpdyma );
