// $Id: plot3d.c 12962 2014-01-27 08:07:41Z airwin $
//
//! @file
//!
//! 3d plot routines.
            ...
            PLINT indexxmin, PLINT indexxmax, const PLINT *indexymin, const PLINT *indexymax )
{
    ...
    PLINT      color = m_plsc->icol0;
    PLFLT      width = m_plsc->width;
    ...
    fc_minz = m_plsc->ranmi;
    fc_maxz = m_plsc->ranma;
    if ( fc_maxz == fc_minz )
    {
        ...
        do  // for each contour level
        {
            ...
            do  // there are several lines that make up each contour
            {
                ...
                if ( cline->npts > 0 )
                {
                    ...
                    {
                        plcol1( ( clev->level - fc_minz ) / ( fc_maxz - fc_minz ) );
                        cx = plP_wcpcx( plP_w3wcx( cline->x[i], cline->y[i], m_plsc->ranmi ) );
                        for ( j = i; j < cline->npts; j++ ) // convert to 2D coordinates
                        {
                            uu[j] = plP_wcpcx( plP_w3wcx( cline->x[j], cline->y[j], m_plsc->ranmi ) );
                            vv[j] = plP_wcpcy( plP_w3wcy( cline->x[j], cline->y[j], m_plsc->ranmi ) );
                        ...
                        if ( j < cline->npts )                    // line not yet finished,
                        {
                            ...
                            {
                                uu[i] = plP_wcpcx( plP_w3wcx( cline->x[i], cline->y[i], m_plsc->ranmi ) );
                                if ( uu[i] > cx )
                                    break;
                                else
                                    cx = uu[i];
