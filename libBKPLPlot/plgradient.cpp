...
c_plgradient( PLINT n, const PLFLT *x, const PLFLT *y, PLFLT angle )
{
    ...
    else
    {
      #define NGRAD    2
        ...
        m_plsc->xgradient = xgrad;
        m_plsc->ygradient = ygrad;
        m_plsc->ngradient = NGRAD;
        ...
        plP_plfclp( xpoly, ypoly, n, m_plsc->clpxmi, m_plsc->clpxma,
            m_plsc->clpymi, m_plsc->clpyma, plP_gradient );
...
gradient_defined( PLFLT x, PLFLT y )
{
    return plP_pointinpolygon( m_plsc->n_polygon, m_plsc->x_polygon, m_plsc->y_polygon,
        x, y );
}
