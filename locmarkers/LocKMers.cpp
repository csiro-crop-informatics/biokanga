...
int
CLocKMers::LocKMers(etPMode PMode,		// processing mode - currently unused, defaults to 0
...
for(Idx = 0; Idx < NumPartialCultivars; Idx++,pPartialCultivar++)
	...
	strncpy(pPartialCultivar->szEntryName,pszPartialCultivars[Idx],sizeof(pPartialCultivar->szEntryName));
...
// now report the non-targeted chrom names
int EntryID;
char szSfxEntryName[100];
for(EntryID = 1; EntryID <= m_NumSfxEntries; EntryID++)
	{
	if(IsTargetCultivar(EntryID))
		continue;
...
gDiagnostics.DiagOut(eDLInfo,gszProcName,"Progress - K-Mers processed: %lld, Cultivar specific : %u, Hamming retained: %u, Extended: %u",NumKMers,NumPutativeKMers, NumAcceptedKMers,NumAcceptedExtdKMers);
...
NumAcceptedExtdKMers = GetKMerProcProgress(&NumBlocks,&NumKMers,&NumPutativeKMers, &NumAcceptedKMers);
gDiagnostics.DiagOut(eDLInfo,gszProcName,"Completed - K-Mers processed: %lld, Cultivar specific : %u, Hamming retained: %u, Extended: %u",NumKMers,NumPutativeKMers, NumAcceptedKMers,NumAcceptedExtdKMers);
...
while((SubSeqsLen=GetBlockSeqs(pPars->AllocBlockSeqsSize - 1,pPars->pBlockSeqs)) > 0)  // -1 because a eBaseEOG will be later appended
	...
	do {
		...
		for(StartIdx = 0; StartIdx <= CurSeqLen - m_KMerLen; StartIdx++, pStartKMerBase++)
			...
			NxtHitIdx = 0;
			RcplHitIdx = 0;
			memcpy(RevCplKMer,pStartKMerBase,m_KMerLen);
