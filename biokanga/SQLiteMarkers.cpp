...
//     SrcCnts VARCHAR(1),					-- From where the counts were derived - 'S' SNP call, 'I' imputed from coverage or from SAM/BAM alignment sequences
...
tsStmSQL CSQLiteMarkers::m_StmSQL[7] = {
	...
	{ (char *)"TblSnps",	
		(char *)"CREATE TABLE TblSnps (SnpID INTEGER PRIMARY KEY ASC,ExprID INTEGER,CultID INTEGER,LociID INTEGER,SrcCnts VARCHAR(1),Acnt INTEGER DEFAULT 0,Ccnt INTEGER DEFAULT 0,Gcnt INTEGER DEFAULT 0,Tcnt INTEGER DEFAULT 0,Ncnt INTEGER DEFAULT 0,TotCovCnt INTEGER DEFAULT 0,TotMMCnt INTEGER DEFAULT 0)",
		(char *)"INSERT INTO TblSnps (ExprID,CultID,LociID,SrcCnts,Acnt,Ccnt,Gcnt,Tcnt,Ncnt,TotCovCnt,TotMMCnt) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
		NULL,
...
if((sqlite_error = sqlite3_bind_text(pStm->pPrepInsert, 4, szSrcCnts,2,SQLITE_STATIC))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
...
if((sqlite_error = sqlite3_bind_int(pStm->pPrepInsert, 5, Acnt))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
...
if((sqlite_error = sqlite3_bind_int(pStm->pPrepInsert, 6, Ccnt))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
	}
if((sqlite_error = sqlite3_bind_int(pStm->pPrepInsert, 7, Gcnt))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
	}
if((sqlite_error = sqlite3_bind_int(pStm->pPrepInsert, 8, Tcnt))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
...
if((sqlite_error = sqlite3_bind_int(pStm->pPrepInsert, 10, TotCovCnt))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
	}
if((sqlite_error = sqlite3_bind_int(pStm->pPrepInsert, 11, TotMMCnt))!=SQLITE_OK)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"sqlite - bind prepared statement: %s", sqlite3_errmsg(m_pDB)); 
	CloseDatabase(true);
	return(eBSFerrInternal);
...
while((Rslt=pCSV->NextLine()) > 0)			// onto next line containing fields
	...
	switch(CSVtype) {
		case 0:					// markers
			...
			if(NumFields < 13 || NumFields != (NumCultivars * 9) + 4)
				{
				gDiagnostics.DiagOut(eDLFatal,gszProcName,"Expected Marker CSV file number of fields to be ((NumCultivars * 9) + 4) in '%s', GetCurFields() returned '%d'",pszInFile,NumFields);
				delete pCSV;
				CloseDatabase(true);
		...
		case 1:					// SNPs
			if(NumFields != 23)
				{
				gDiagnostics.DiagOut(eDLFatal,gszProcName,"Expected 'biokanga align' generated SNP CSV file number of fields to be 23 at line %d in '%s', GetCurFields() returned '%d'",NumElsRead,pszInFile,NumFields);
				delete pCSV;
				CloseDatabase(true);
				return(eBSFerrFieldCnt);
				}
	...
	switch(CSVtype) {
		case 0:					// markers
			...
			FieldIdx = 5;
			NumCovd = 0;
			for(SpeciesIdx = 0; SpeciesIdx < NumSpecies; SpeciesIdx++,FieldIdx += 9)
				{
				pCSV->GetText(FieldIdx,&pszCntSrc);				// "I" if base counts were imputed, "S" if from SNP call
				CntSrc = *pszCntSrc; 
				pCSV->GetText(FieldIdx+1,&pszMarkerBase);		// marker base
				...
				pCSV->GetInt(FieldIdx+6,&Gcnt);
				pCSV->GetInt(FieldIdx+7,&Tcnt);
				pCSV->GetInt(FieldIdx+8,&Ncnt);
				switch(LociBase) {
					case 'A':
				...
				if(TotCovCnt > 0 || MarkerScore > 0)
					{
					SnpID = AddSNP(ExprID,CultID,LociID,CntSrc,Acnt,Ccnt,Gcnt,Tcnt,Ncnt,TotCovCnt,TotMMCnt);
		...
		case 1:					// SNPs
			...
			if(TotCovCnt) {
				...
				SnpID = AddSNP(ExprID,CultID,LociID,'S',Acnt,Ccnt,Gcnt,Tcnt,Ncnt,TotCovCnt,TotMMCnt);
				}
			break;
		}
	}
